...
describe('Players', () => {
    ...
    const playerWithNoImage = Object.fromEntries(
        Object.entries(currentPlayer!).filter(([ key ]) => key !== 'image'),
    ) as Player
    ...
    it('should add currentPlayer state to the current user', () => {
        ...
        expect(
            driver
                .styleUtils()
                .hasStyleState(playerTestkit.element()!, 'currentPlayer'),
        ).toBeTruthy()
    })
    ...
    it('should show avatar component when image not defined', () => {
        ...
        expect(driver.testkit().player(0).avatar().image().src()).toContain(
            encodeURI(playerWithNoImage.name),
        )
    })
})
