...
describe('SelectSubject', () => {
	...
	const setSubject = jest
		.fn()
		.mockImplementation((aSubject: string | undefined) => { subject = aSubject })

	const driver = selectSubjectDriver({ props: { subject, setSubject } }).beforeAndAfter()
	...
	it('should render subjects', () => {
		...
		for (const subjectValue of Object.values(GameSubjects)) {
			expect(driver.testkit().subject(subjectValue).text()).toEqual(toTitleCase(subjectValue))
		}
	})
	...
	it('should filter subject according to text', () => {
		...
		expect(driver.testkit().subject(expectedSubject).element()).toBeDefined()
		expect(driver.testkit().subject(expectedMissingSubject).element()).toBeNull()
	})
