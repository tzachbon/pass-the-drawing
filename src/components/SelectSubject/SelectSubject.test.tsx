...
describe('SelectSubject', () => {
  ...
  const setSubject = jest.fn().mockImplementation(aSubject => { subject = aSubject })
  const testkit = renderer(() => <SelectSubject subject={subject} setSubject={setSubject} />).beforeAndAfter()
  ...
  it('should render subjects', async () => {
    ...
    for (const subjectValue of Object.values(GameSubjects)) {
      expect(
        await testkit.container.findByTestId(getSubjectTestId(subjectValue))
  ...
  it('should filter subject according to text', async () => {
    ...
    expect(await testkit.container.findByTestId(getSubjectTestId(GameSubjects.Food))).toBeDefined()
    expect(testkit.container.queryByTestId(getSubjectTestId(GameSubjects.Cars))).toBeNull()
  })
