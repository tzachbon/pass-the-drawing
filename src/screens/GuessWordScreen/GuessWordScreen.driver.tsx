...
export function guessWordScreenTestkit(container: RenderResult) {
	...
	const testkit = {
		...utils,
		title: () => testUtils(TITLE_TEST_ID, container, { keys: [ 'text', 'disabled' ] }),
		input: () => testUtils(INPUT_TEST_ID, container, { keys: [ 'type', 'placeholder', 'disabled', 'value' ] }),
		button: () => testUtils(BUTTON_TEST_ID, container, { keys: [ 'click', 'disabled' ] }),
		// eslint-disable-next-line @typescript-eslint/no-unsafe-return
		draws: () => [ ...testUtils(DRAWS_CONTAINER_TEST_ID, container)._element().children ].map((_, i) => testkit.draw(i)),
		draw: (index: number) => {
