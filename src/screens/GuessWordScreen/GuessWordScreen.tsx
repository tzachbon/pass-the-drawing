...
export const getDrawTestId = (index: string) =>
    'GuessWordScreen_DRAW_TEST_ID_' + index
...
export const GuessWordScreen: React.VFC<GuessWordScreenProps> = ({
    ...
    return (
        <form
            ...
            onSubmit={onSubmit}
        >
            <h1 data-testid={TITLE_TEST_ID}>Guess the word!</h1>
            <input
            ...
            >
                {game.players
                    .filter(
                        (player) =>
                            Boolean(player.draw) && isValidJson(player.draw),
                    )
                    .map((player, i) => (
                        <div
