...
import {
    ROOT_TEST_ID,
    SUBMIT_BUTTON_TEST_ID as SIGN_IN_SUBMIT_BUTTON_TEST_ID,
...
} from '@pages/CreateGame/CreateGame'
...
describe('StartGame', () => {
    ...
    it('should go to create a game', async () => {
        ...
        const openModalButton = await page.waitForSelector(
            testIdToSelector(OPEN_MODAL_BUTTON_TEST_ID),
            { visible: true },
        )
        await openModalButton!.click()
        await page.waitForSelector(testIdToSelector(ROOT_TEST_ID))

        await expect(page).toFillForm(testIdToSelector(ROOT_TEST_ID), {
            ...user,
        })
        ...
        await expect(page).toMatchElement(
            testIdToSelector(LOGGED_IN_MESSAGE_TEST_ID),
            { text: `Logged in as ${user!.email}` },
        )

        const input = await page.waitForSelector(
            testIdToSelector(INPUT_TEST_ID),
        )
        await input!.type('Food')
        await input!.evaluate((el) =>
            (el as unknown as HTMLInputElement).blur(),
        )

        const submitButton = await page.waitForSelector(
            testIdToSelector(SUBMIT_BUTTON_TEST_ID),
        )
